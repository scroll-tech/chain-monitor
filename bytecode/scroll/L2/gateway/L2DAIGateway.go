...
var (
	...
	// L2DAIGatewayABI is the input ABI used to generate the binding from.
	L2DAIGatewayABI, _ = L2DAIGatewayMetaData.GetAbi()
)
...
func (o *L2DAIGateway) ParseLog(vLog *types.Log) (bool, error) {
	_id := vLog.Topics[0]
...
func (o *L2DAIGateway) RegisterUpdateRateLimiter(handler func(vLog *types.Log, data *L2DAIGatewayUpdateRateLimiterEvent) error) {
	_id := o.ABI.Events["UpdateRateLimiter"].ID
	o.parsers[_id] = func(log *types.Log) error {
		event := new(L2DAIGatewayUpdateRateLimiterEvent)
		if err := o.L2DAIGatewayCaller.contract.UnpackLog(event, "UpdateRateLimiter", *log); err != nil {
			return err
		}
		event.Log = log
		return handler(log, event)
...
func (_L2DAIGateway *L2DAIGatewayCaller) RateLimiter(opts *bind.CallOpts) (common.Address, error) {
	var out []interface{}
	err := _L2DAIGateway.contract.Call(opts, &out, "rateLimiter")
...
func (_L2DAIGateway *L2DAIGatewayTransactor) UpdateRateLimiter(opts *bind.TransactOpts, _newRateLimiter common.Address) (*types.Transaction, error) {
	return _L2DAIGateway.contract.Transact(opts, "updateRateLimiter", _newRateLimiter)
}
...
// L2DAIGatewayUpdateRateLimiter represents a UpdateRateLimiter event raised by the L2DAIGateway contract.
type L2DAIGatewayUpdateRateLimiterEvent struct {
	Log *types.Log `json:"-" gorm:"-"`
