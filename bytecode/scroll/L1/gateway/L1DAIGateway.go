...
var (
	...
	// L1DAIGatewayABI is the input ABI used to generate the binding from.
	L1DAIGatewayABI, _ = L1DAIGatewayMetaData.GetAbi()
)
...
func (o *L1DAIGateway) RegisterUpdateRateLimiter(handler func(vLog *types.Log, data *L1DAIGatewayUpdateRateLimiterEvent) error) {
	_id := o.ABI.Events["UpdateRateLimiter"].ID
	o.parsers[_id] = func(log *types.Log) error {
		event := new(L1DAIGatewayUpdateRateLimiterEvent)
		if err := o.L1DAIGatewayCaller.contract.UnpackLog(event, "UpdateRateLimiter", *log); err != nil {
			return err
		}
		event.Log = log
		return handler(log, event)
...
func (_L1DAIGateway *L1DAIGatewayCaller) RateLimiter(opts *bind.CallOpts) (common.Address, error) {
	var out []interface{}
	err := _L1DAIGateway.contract.Call(opts, &out, "rateLimiter")
...
func (_L1DAIGateway *L1DAIGatewayTransactor) UpdateRateLimiter(opts *bind.TransactOpts, _newRateLimiter common.Address) (*types.Transaction, error) {
	return _L1DAIGateway.contract.Transact(opts, "updateRateLimiter", _newRateLimiter)
}
...
// L1DAIGatewayUpdateRateLimiter represents a UpdateRateLimiter event raised by the L1DAIGateway contract.
type L1DAIGatewayUpdateRateLimiterEvent struct {
	Log *types.Log `json:"-" gorm:"-"`
