...
// SetL1WatcherAPI set l1 watcher api.
func (l2 *L2Watcher) SetL1WatcherAPI(l1Watcher controller.WatcherAPI) {
	l2.filter.l1gatewayCfg = l1Watcher.GetGatewayConfig()
}
...
func (l2 *L2Watcher) checkReorg(ctx context.Context) (*types.Header, error) {
	...
	if err = deleteReorgEvents(ctx, l2.db, reorgNumbers, l2.filter.chainName); err != nil {
		return nil, err
	}
...
func deleteReorgEvents(ctx context.Context, db *gorm.DB, numbers []uint64, chainName string) error {
	...
	log.Warn("L2 chain reorg", "reorg numbers", numbers)
